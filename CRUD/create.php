...
/*		if (empty($end_date)) {            // removed so that end_date can be null
        ...
        if ($valid) {
			...
			$pdo = Database::connect();
        	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql2 = "INSERT INTO main (acronym) VALUES (?)";
			$q2 = $pdo -> prepare($sql2);
			$q2 -> execute(array($acronym));
        	Database::disconnect();
...
<body>
   <div class="container">
             ...
             <a href="https://www3.acs.ncsu.edu/Shibboleth.sso/Logout" class="btn btn-danger">Logout</a>
                    ...
                    <form class="form-horizontal"  role= "form" action="create.php?id=<?php echo $id?>" method="post">
